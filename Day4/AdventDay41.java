...
package adventday4.pkg1;

import java.util.Arrays;


public class AdventDay41 {
...
int validCount=0; 
int invalidCount=0;
int lineCount = nCount(input);
        
for(int i=0; i<lineCount;i++){
    String line = (input.split("\n+")[i].trim());//working
    String[] splitLine = line.split(" ");
    int similar = isSame(splitLine);    //If there are similarties 
    if(similar>0){
        invalidCount++;
        }
    }
        validCount=lineCount-invalidCount;
        System.out.println("ANSWER: "+(validCount));

}

    public static int isSame(String[] splitLine){
        int same=0;
        for(int x=0; x<splitLine.length;x++){
        for(int y=0; y<splitLine.length;y++){
           char[] l1 = splitLine[x].toCharArray();
           char[] l2 = splitLine[y].toCharArray();
//           Arrays.sort(l1); //Isn't needed - this searched for anagrams eg/ abcd = dcba where I should be searching for abcd = abcd
//           Arrays.sort(l2);
           if(Arrays.equals(l2, l1)&&x!=y){
               same++;
           }
        }
     }
    ...
    public static int nCount(String toCount){
    int count = toCount.split("\r?\n").length; //How many \n's in the string 
    return count;
    }
}
