...
package adventdat7.pkg2;

import java.util.ArrayList;
import java.util.LinkedHashSet;

public class AdventDay72 {

    public static void main(String[] args) {
        String input="rmhcw (26)\n" +
"dmmriu (61)\n" +
...
"baomcxj (54)";
         ...
         String input2 = input;    //Used for part 2
        input=input.replace("\n", "/");
        input=input.replace("(", "");   //Formatting and splitting input string into usable format
        input=input.replace(")", "");
        input=input.replace(",", "-");
        input=input.replace(" ", "");
        for(int i=0; i<=9;i++){ //removing numbers, not needed for pt1
            input=input.replace(Integer.toString(i), "");
        }
        
        String[] splitString=input.split("/");
        ArrayList<String> mainArray = new ArrayList<>(); //More splitting eh
        for(int i=0;i<splitString.length;i++){
            mainArray.add(splitString[i]);
        }
        ...
        for(int i=0;i<splitString2.length;i++){
            weightArray.add(splitString2[i]);
        }
        
//        System.out.println(weightArray.get(428));
//        System.out.println(weightArray.get(1360));
//        System.out.println(weightArray.get(268));
        
        for(int i=0;i<mainArray.size();i++){
            int initialWeight=(weightOf(i,weightArray));
            if(mainArray.get(i).contains("->")){
                int stackWeight=0;
                String inspect=mainArray.get(i);
                String[] splitInspect = inspect.split("->");
                String hold = splitInspect[splitInspect.length-1];
                String[] split2 = hold.split("-");
                ArrayList<String> indexToFindWeight = new ArrayList<>();
                boolean flag=false;
                for(int x=0;x<split2.length;x++){
                  //  flag=false;
                        for(int y=0;y<mainArray.size();y++){
                            if(mainArray.get(y).contains(split2[x])&&mainArray.get(y).contains("->")){
                                flag=true;
                                indexToFindWeight.add(Integer.toString(y));
                        }
                            else if(mainArray.get(y).contains(split2[x])&&!mainArray.get(y).contains("->")){
                                indexToFindWeight.add(Integer.toString(y));
                            }
                    }
                }
                if(flag==true){
                    indexToFindWeight=removeDupe(indexToFindWeight);
                    indexToFindWeight.remove(0);
                }
                System.out.println("INDEXS TO FIND WEIGHT "+indexToFindWeight);
                for(int x=0;x<indexToFindWeight.size();x++){
                    stackWeight+=weightOf(Integer.parseInt(indexToFindWeight.get(x)),weightArray);
                }
                System.out.println("WEIGHT OF PROGRAM: "+mainArray.get(i));
                System.out.println("INITAL WIEGHT: "+initialWeight);
                System.out.println("STACK WEIGHT: "+stackWeight);
            if((weightOf(i,weightArray))<stackWeight){
        ...
        ArrayList<Integer> resultMem = new ArrayList<>();   //Arraylist of memory to hold chain or results 
        while(result!=1){  //Using a while loop as a kinda 'fake recursion'
            int x=newLowIndex(mainArray,result);    //Calling func to find new lowest index
            result=x;
            resultMem.add(result);  //Adding new lowest index to memory, while loop will repeate if !=1
        }
        int resultIndex = resultMem.size()-2;   //Getting final 'real' result (not 1)
        if(result==1){  //Bottom of tree found if result==1
            System.out.println("########FINAL RESULT#############");
            System.out.println(""+mainArray.get(resultMem.get(resultIndex)));   //Printing element name
            System.out.println("           "+resultMem.get(resultIndex));   //Printing element index
            System.out.println("#################################");
        }
    }
    
    
    public static int newLowIndex(ArrayList<String> mainArray, int currentLowest){
        int newLowestIndex; //New index to be returned 
        String hold=""; //Hold = the data after the -> of the string (basicallt the tail on a node
        String inspectParam=mainArray.get(currentLowest);   //Current val being passed in by params
        String[] splitInspectParam = inspectParam.split("->");  //Splitting by tail
        String headParam = splitInspectParam[0];    //Selecting just the 'head' - we need to search by head for matches not tail matches
        
        for(int i=0;i<mainArray.size();i++){    //Looping through array
            
            if(mainArray.get(i).contains("->")){    //if node has a tail with data
                String inspect=mainArray.get(i);
                String[] splitInspect = inspect.split("->");
                hold = splitInspect[splitInspect.length-1]; //Get tail data and put into hold
            }

            if(i!=currentLowest && hold.contains(headParam)){   //If the header (what we're searching for) appears in the hold data of any string
                newLowestIndex=i;   //Returning next index location of match
                return newLowestIndex;
            }
        }
        return 1;   //if no data found - this means that it's the base as it doesn't appear in a tail!
    }
    
    public static int weightOf(int index, ArrayList<String> weightArray){
        String toCut = weightArray.get(index);
        toCut = toCut.replaceAll("[^\\d.]", "");
        int weight=Integer.parseInt(toCut);
        return weight;
    }
    
    public static ArrayList<String> removeDupe(ArrayList<String> inList){
        ArrayList<String> list = new ArrayList<>();
        for(int i=0;i<inList.size();i++){
        if(!list.contains((inList).get(i))){
           list.add(inList.get(i));
        }
    }
        System.out.println(list+" LIST NO DUPES");
        return list;
    }
}